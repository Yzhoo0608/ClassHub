...
if ($conn->connect_error) {
    die("Connection failed: " . $conn->connect_error); // Stop if DB connection fails
}

// Login Logic
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    $email = trim($_POST['email']); // Get and sanitize email
    $password = trim($_POST['password']); // Get and sanitize password
    $role = $_POST['role']; // Determine role: user or admin

    // Choose the correct table and columns based on role
    if ($role == 'user') {
    ...
    // Verify password and handle login success or failure
    if ($user && password_verify($password, $user['password'])) {
        ...
        $error = "Invalid email or password!"; // Show error on failed login
    }
}
...
<body>
  ...
  <div class="main_box">
    <!-- Left section with branding and role selection -->
    <div class="left_box">
    ...
    <div class="login">
      ...
      <form method="POST">
        <input type="hidden" id="selected_role" name="role" value="user"><!-- Hidden role input -->
  ...
  <script>
    // JavaScript for checkbox role selection
    ...
    function updateRoleSelection(clickedCheckbox) {
      if (clickedCheckbox === userCheckbox) {
        ...
        registerLink.style.display = "block"; // Show register link for users
      } else {
        user_checkbox.checked = false;
        admin_checkbox.checked = true;
        roleInput.value = "admin";
        registerLink.style.display = "none"; // Hide register link for admins
      }
    }
