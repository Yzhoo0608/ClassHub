...
// Based on the email to get the user id from the users table
...
// Get or create a classroom id based on room name, date, and time slot
function getClassroomId($conn, $room_name, $date, $time_slot) {
    ...
    // If no classroom exists and create one
    if (!$classroom_id) {
...
// If the time slot is not already taken so this is to insert a new booking into the database
function insertBooking($conn, $name, $email, $room_name, $date, $time_slot, $reason, $status = 'Pending') {
    ...
    // This is to insert booking in a transaction
...
document.addEventListener("DOMContentLoaded", function () {
    ...
    const timeSlotInput = document.getElementById("time_slot");
     ...
     // Fetch and update booked time slots based on selected classroom and date
            ...
            .then(data => {
                timeSlotDivs.forEach(div => {
                    ...
                    if (data.includes(val)) {
                        div.classList.add("booked"); // disable booked slot
                    }
                });
            });
    }

    // Disable classroom options that are fully booked
        ...
        fetch(`user_booking.php?ajax=room_availability&date=${encodeURIComponent(selectedDate)}`)
            ...
            .then(data => {
                for (const room in data) {
                    ...
                    if (option) {
                        // Hide room if it has 4 and all time of  a room is booked
    ...
    // Allow time slot selection only if not booked
    timeSlotDivs.forEach(div => {
    ...
    // This is to update slots and room availability on change
    ...
    // Validate time slot is selected before submitting
