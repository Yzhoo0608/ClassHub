...
// Check if user is logged in (session user_id must exist)
if (!isset($_SESSION['user_id'])) {
    header("Location: login.php"); // Redirect to login page if not logged in
    exit(); // Stop script execution
}
// Get the page parameter from URL, default to 'calendar_G1' if not set
...
if ($_SERVER["REQUEST_METHOD"] === "POST" && $_POST['action'] === 'processBooking') {
    ...
    // ------------------ Get classroom ID based on room name ------------------
    ...
    if (!$classroom_id) {
        ...
        if ($count > 0) {
            ...
            if ($user_id_found) {
                // User exists, insert booking into database
...
<div class="sidebar">
    ...
    <b>Welcome</b><br>
    <!-- Submenu for classroom calendars -->
...
<script>
// Function to toggle the visibility of a submenu when its parent is clicked
function toggleSubmenu(event) {
    event.preventDefault();
    const parent = event.target.closest('.has-submenu');
    // Toggle the 'active' class on the parent to show or hide the submenu
